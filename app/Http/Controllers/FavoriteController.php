...
class FavoriteController extends Controller
{
    ...
    public function like_photo(Request $request)
    {
        ...
        if ($check_photo) {
            ...
            FavoriteModel::where('photo_id', $request['photo']['id'])->create([
    ...
    {
        FavoriteModel::where('image_id', $request['photo']['id'])->where('user_id', 4)->delete();
        return response(['success' => true, 'message' => "photo_was_removed"]);
    }

    public function get_liked_photos(Request $request)
    {
        $liked_photos = FavoriteModel::where('user_id', 4)->select('image_id', 'user_id')->get();
